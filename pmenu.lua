...
function JMenu:resetMenu()
	...
	self.Base = {
		...
		Title = JCore and JCore.user and JCore.user.name or "Menu",
...
function JMenu:stringsplit(inputstr, sep)
...
function JMenu:CloseMenu(bypass)
...
function JMenu:GetButtons(customMenu)
...
function JMenu:OpenMenu(stringName, boolBack)

	if stringName and not self.Menu[stringName] then print("[JMenu] " .. stringName .. " (erreur)") return end
...
function JMenu:CreateMenu(tableMenu, tempData)
...
function JMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	local stringPrefix = (((button.r and (((JCore and (JCore.jobRank < button.r or JCore.copRank < button.r)) or (button.rfunc and not button.rfunc())) and "~r~" or "")) or "") .. (self.Events["setPrefix"] and self.Events["setPrefix"](button, self.Data) or "")) or ""
...
function JMenu:DrawButtons(tableButtons)
...
function JMenu:DrawHeader(intCount)
...
function JMenu:DrawHelpers(tableButtons)
...
function JMenu:DrawExtra(tableButtons)
	ShowCursorThisFrame()
...
function CloseMenu(force)
	return JMenu:CloseMenu(force)
end

function CreateMenu(arrayMenu, tempData)
	return JMenu:CreateMenu(arrayMenu, tempData)
end

function OpenMenu(stringName)
	return JMenu:OpenMenu(stringName)
end
...
Citizen.CreateThread(function()
	while true do
		...
		if JMenu.IsVisible and not JMenu.Invisible then
			JMenu:ProcessControl()
		end
	end
end)
